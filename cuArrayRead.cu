...
__global__ void copyImagecuArray( uchar * dst, uint imageH, uint imageW, uint pitch, uint channel)
{
...
bool testcuArrayRead(cv::Mat &image, std::string  titile)
{
	...
	TIMED("malloc device dst memory,and clear it!!")
	{
		if (image.type() == CV_8UC3)
		{
			checkCudaErrors(cudaMallocArray(&cuArray, &cf,
				size_width, /////in pixel!!!!
				size_height));
		}
		else if (image.type() == CV_8UC4)
		{
			checkCudaErrors(cudaMallocArray(&cuArray, &cf,
				size_width / (sizeof(uchar)* image.channels()), /////in pixel!!!!
				size_height));
		}
	...
	if (image.type() == CV_8UC3)
	{
		...
		checkCudaErrors(cudaBindTextureToArray(&tex, cuArray, &cf));
	...
	{
		// bind texture reference to array
		checkCudaErrors(cudaBindTextureToArray(&tex4, cuArray, &cf));
		TIMED("malloc host dst memory")
		{
	...
	TIMED("cudaMemcpyHostToDevice")
	{
		...
		checkCudaErrors(cudaMemcpyToArray(cuArray, 0, 0, image.data, size_width*size_height, cudaMemcpyHostToDevice));
	}
	...
	TIMED("copyImage")
	{
		for (int i = 0; i < 10000; i++)
		{
			...
			copyImagecuArray << <gridSize, blockSize >> >(devPtr_dst, (uint)height, (uint)width, (uint)size_pitch, (uint)channel);
